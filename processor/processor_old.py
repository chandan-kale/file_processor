...
def process_with_pandas(file_path, output_dir, rows_per_file):
    ...
    for i in range(num_chunks):
        ...
        if ext == '.xlsx':
            chunk.to_excel(os.path.join(output_dir, f'part_{i+1}.xlsx'), index=False)
        elif ext == '.csv':
...
def process_with_polars(file_path, output_dir, rows_per_file):
    ...
    for i in range(num_chunks):
        ...
        if ext == '.xlsx':
            chunk.write_excel(os.path.join(output_dir, f'part_{i+1}.xlsx'))
        elif ext == '.csv':
            chunk.write_csv(os.path.join(output_dir, f'part_{i+1}.csv'))
